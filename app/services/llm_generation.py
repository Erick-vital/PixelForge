        ...
        try:
            ...
            if requested_model != default_model and exc.status_code in {400, 404, 422}:
                logger.warning(
                    ...
                    extra={"provider": resolved_provider, "requested_model": requested_model, "fallback_model": default_model, "status_code": exc.status_code},
                )
                text = await self._generate_once(
                    system_prompt=system_prompt,
                    prompt=prompt,
            ...
            logger.warning("llm generation provider failed without fallback", extra={"provider": resolved_provider, "model": requested_model, "status_code": exc.status_code})
            raise

    async def _generate_once(self, *, system_prompt: str, prompt: str, provider: str, model: str, base_url: str, api_key: str, temperature: float, max_tokens: int | None) -> str:
        if provider == "anthropic":
            return await _post_anthropic_message(system_prompt=system_prompt, prompt=prompt, api_key=api_key, model=model, base_url=base_url, timeout_seconds=self.timeout_seconds, max_tokens=max_tokens or 2000)
        if provider in {"openai", "openai_compatible"}:
            return await _post_openai_chat_completion(system_prompt=system_prompt, prompt=prompt, api_key=api_key, model=model, base_url=base_url, timeout_seconds=self.timeout_seconds, temperature=temperature, max_tokens=max_tokens)
        raise LlmGenerationProviderError(f"Unsupported LLM provider: {provider}")


async def _post_openai_chat_completion(*, system_prompt: str, prompt: str, api_key: str, model: str, base_url: str, timeout_seconds: int, temperature: float, max_tokens: int | None) -> str:
    endpoint = base_url.rstrip("/") + "/chat/completions"
    payload: dict[str, Any] = {"model": model, "messages": [{"role": "system", "content": system_prompt}, {"role": "user", "content": prompt}], "temperature": temperature}
...
async def _post_anthropic_message(*, system_prompt: str, prompt: str, api_key: str, model: str, base_url: str, timeout_seconds: int, max_tokens: int) -> str:
    endpoint = base_url.rstrip("/") + "/v1/messages"
    payload = {"model": model, "max_tokens": max_tokens, "system": system_prompt, "messages": [{"role": "user", "content": prompt}]}
    ...
    texts = [str(part["text"]) for part in parts if isinstance(part, dict) and part.get("type") == "text" and part.get("text")]
    return _normalized_content("".join(texts))


async def _post_json(*, endpoint: str, headers: dict[str, str], payload: dict[str, Any], timeout_seconds: int, provider: str, model: str) -> httpx.Response:
    logger.info("llm provider request started", extra={"provider": provider, "model": model, "endpoint": endpoint, "timeout_seconds": timeout_seconds})
    try:
        async with httpx.AsyncClient(timeout=timeout_seconds) as client:
            response = await client.post(endpoint, headers=headers, json=payload)
            response.raise_for_status()
            logger.info("llm provider request completed", extra={"provider": provider, "model": model, "endpoint": endpoint, "status_code": response.status_code})
            return response
    except httpx.HTTPStatusError as exc:
        status_code = exc.response.status_code if exc.response is not None else None
        response_text = _safe_response_text(exc.response)
        logger.warning("llm provider request returned error status", extra={"provider": provider, "model": model, "endpoint": endpoint, "status_code": status_code, "response_body": response_text})
