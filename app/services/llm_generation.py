        ...
        *,
        ...
        try:
            ...
            logger.warning(
                ...
                extra={"provider": resolved_provider, "model": requested_model, "status_code": exc.status_code},
            )
            raise
    ...
    *,
    ...
    try:
        ...
        raise LlmGenerationProviderError(
            "LLM provider returned an unexpected OpenAI-compatible response shape"
        ) from exc
    return _normalized_content(content)


async def _post_anthropic_message(
    *, system_prompt: str, prompt: str, api_key: str, model: str, base_url: str, timeout_seconds: int, max_tokens: int
) -> str:
