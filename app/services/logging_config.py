...
def _safe_make_record(self, name, level, fn, lno, msg, args, exc_info, func=None, extra=None, sinfo=None):
    if extra:
        extra = {(f"extra_{key}" if key in _RESERVED_LOG_RECORD_FIELDS else key): value for key, value in extra.items()}
...
def configure_logging(level: int | str | None = None) -> None:
    ...
    resolved_level = level or os.getenv("APP_LOG_LEVEL", "INFO")
